...
exports.kickoff = async function(event, context) {
    ...
    for (const blockDevice of image.BlockDeviceMappings) {
        ...
        const accountIds = process.env.SHARE_WITH_ACCOUNTS.split(',');
        console.log(`Sharing snapshot ID ${snapshotId} with accounts ${process.env.SHARE_WITH_ACCOUNTS} ...`);
...
exports.notifySuccess = async function(event, context) {
    let jobId;
    // The event is likely to be an array since it includes the results
    // of the parallel Tasks that preceded it.
    ...
    console.log(`Sending Success Result to job ${jobId}`);
...
exports.notifyFailure = async function(event, context) {
    ...
    await new aws.CodePipeline().putJobFailureResult({
        ...
        failureDetails: {
            type: 'JobFailed',
            message: `Error occurred during copy of snapshot to ${event.destinationRegion}, snapshot ID ${event.destinationSnapshotId}`
        }
    }).promise();
};
